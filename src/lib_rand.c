...
// a single global one
hby_StructMethod rand_mod[] = {
  {"seed", rand_seed, 0, hby_static_fn},
  {"next", rand_next, 0, hby_static_fn},
  {"irange", rand_irange, 2, hby_static_fn},
  {"frange", rand_frange, 2, hby_static_fn},
  {NULL, NULL, 0, 0},
};

bool open_rand(hby_State* h, int argc) {
  hby_push_struct(h, "rand");
  hby_add_members(h, rand_mod, -2);
  hby_set_global(h, "rand");
  hby_pop(h, 2);
